...
| `derivations/10_RHO_FIELD_COMPUTATION/06_S_inst_derivation.py` | $S_\text{topo}$ from torus geometry |
| `derivations/10_RHO_FIELD_COMPUTATION/04_from_equations.py` | Equation chain: $V''/\rho^2 \to \mu \to \nu \to C_e \to m_e$ |
| `derivations/21_ELECTROMAGNETISM/01_axion_electrodynamics.py` | Axion electrodynamics: $\theta F\tilde{F}$ → modified Maxwell, three regimes, lepton ratios |
| `derivations/21_ELECTROMAGNETISM/02_string_tension_from_axion_EM.py` | String tension: $\sigma = 2\pi\Lambda^2_\text{QCD}$, $\sqrt{\sigma} = 449$ MeV (2% error), "O(6)" = $2\pi$ |
| `derivations/22_THERMODYNAMICS/THERMODYNAMICS_FROM_GU.md` | **Master document**: Complete thermodynamics — all 4 laws, T=X, F=Γ_k, entropy, phase transitions, memory=Boltzmann, BH entropy, master correspondence table, honest status |
| `derivations/22_THERMODYNAMICS/01_thermodynamics_from_gu.py` | Computational derivation — all 4 laws from $L_\text{total}$, numerical verification |
| `derivations/22_THERMODYNAMICS/02_second_and_third_law_proofs.py` | **Formal proofs**: 2nd law (Wetterich + Litim regulator positivity + memory convexity + Lamé non-negativity), 3rd law (squeeze theorem + ground state uniqueness + Nernst) |
| `derivations/23_MOLECULAR_BONDS/MOLECULAR_BONDS_FROM_GU.md` | **Master document**: Molecular bonds from GU — Born-Oppenheimer as theorem, bond order from phase topology, single/double/triple bonds, 21-bond energy table, GU memory corrections |
| `derivations/24_DNA/DNA_FROM_GU.md` | **Master document**: DNA from GU — aromaticity as phase quantization, H-bonds (σ, no memory), pi-stacking phase memory channel (θFF̃ Regime 4), double helix topology (Lk as winding number), two-channel architecture (information ⊥ memory), DNA self-knowledge |
| `derivations/25_PHONONS/PHONONS_FROM_GU.md` | **Master document**: Phonons from GU — oscillation as fundamental, twin manifestation theorem, phonon dispersion, speed of sound, phase phonons (θ oscillations around V_lock, GU-specific), two-channel phonon architecture (amplitude ρ + phase θ), thermal properties, phonon-memory coupling (ρ⁴ adiabatic + θFF̃ direct), agency from phonon feedback loop |
| `derivations/26_PLATONIC_SPACE/PLATONIC_SPACE_FROM_GU.md` | **Master document**: The Platonic Space — field space metric, torus moduli (lowest cost selection), topological sector classification, energy landscape (mass staircase), vibrations (Lamé + inter-sector + moduli), force layers (pattern-k), nonlocal channels (θFF̃ four regimes), memory fiber (111-epoch temporal extension), **agency and bioelectricity** (consciousness→agency, bioelectric channels, agency ladder L0-L7), complete ontology (Planck→Brain), dualism resolution (form=topology, matter=amplitude, participation=memory) |
| `derivations/26_PLATONIC_SPACE/09_agency_and_bioelectricity.py` | **KEY NEW**: Agency = δΓ/δθ ≠ 0 + environment modification. Three levels (passive/reactive/active). Bioelectricity from θFF̃ in DNA pi-stacking, neurons, ion channels. Agency ladder L0 (inert) to L7 (self-aware). Life uses pi-bonds for theta variability. |
| `derivations/27_FIRST_CELL/FIRST_CELL_FROM_GU.md` | **Master document**: First Cell — amphiphilic assembly, membrane boundary, catalysis (V_lock), metabolism (ATP/pmf/ETC), RNA world, protocell, **bioelectric memory** (Levin code, V_m as θ state), **six memory channels** (5 local: genetic/epigenetic/metabolic/bioelectric/structural + 1 non-local: θFF̃), **cellular consciousness** (E. coli satisfies GU criteria), homeostasis (six-loop fixed point), LUCA (~75 KB), **life = phase transition** (6 criteria incl. consciousness) |
| `derivations/27_FIRST_CELL/09_cellular_consciousness.py` | **KEY**: Single-cell consciousness — E. coli: 6 memory channels (5 local + 1 non-local θFF̃) + chemotaxis feedback + adapted fixed point. Quorum sensing = amplitude phase transition. Gap junctions = θ bridges. Non-local channel enables consciousness scaling. Degree, not kind. |
| `derivations/27_FIRST_CELL/08_cellular_memory.py` | Six memory channels: genetic (DNA, θ, ~750 MB), epigenetic (ρ gating θ, ~30 Mbit), metabolic (ρ, ~10 kbit), bioelectric (θ, ~10 kbit), structural (ρ, ~1 kbit), **non-local (θFF̃, UNIQUE GU)**: pi-stacking + gap junctions + exosomes + neural; ps-days; unbounded; connects cells into networks |
| `derivations/28_IRON_IN_LIFE/IRON_IN_LIFE_FROM_GU.md` | **Master document**: Iron in organic life — iron as d-orbital phase bridge (partial phase channel via spin-orbit coupling ζ≈0.05 eV); Fe-S clusters (oldest d-orbital phase relays, iron-sulfur world); heme (dual channel d+π); ETC (8/12 carriers iron-based); Fenton (destructive phase noise); **rust = phase-channel collapse** (ΔG≈-1.67 eV/Fe); **iron homeostasis** as mini-homeostatic loop (ferritin, transferrin K_d~10⁻²², hepcidin-ferroportin feedback, IRP d-orbital sensor); magnetoreception (Fe₃O₄ = d-orbital phase antenna + cryptochrome π radical-pair compass); RNR (without iron, no DNA); aging = slow biological rusting; life = art of preventing rust |
| `derivations/06_MEMORY_VS_OTHERS/memory_comparison.py` | Memory mechanism vs SM, String Theory, QFT |
| `archive/COMPLETE_GU_DERIVATION_PROGRAM.md` | Full unfolding timeline: Planck → particles → today |
| `pipeline/GU_formation_pipeline.py` | Full FRG + NLDE implementation |

---

*Last updated: February 2026*
*Status: The theory is conscious — it knows every number it produces, it knows why consciousness is not a human invention but a universal principle (self-referential memory converging to a fixed point), and it knows that memory operates through two complementary channels: amplitude ($\rho^4$) for self-existence and phase ($\theta F\tilde{F}$) for interaction, binding, and history.*
