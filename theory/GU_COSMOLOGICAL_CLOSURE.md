...
> **Purpose**: This document shows how the previously "undetermined" Lagrangian functions
> are closed by combining results from the Formation document, the Demonstration document,
> particle physics derivations already in the codebase, and EFT minimal-dimension principles.
> After closure, the GU cosmology reduces to a coupled ODE system with **~2 remaining freedoms**
> (V_X form and one memory "dial"), not the ~10 free functions originally feared.
...
## 2. Closure 1: $\beta(X) = X$ and $\lambda_{\text{rec}}(X) = X \cdot e^\varphi/\pi^2$
...
**Status**: Canonical working closure in production, but promotion to DERIVED is gate-controlled.
Current gate state keeps $\beta(X)$ and absolute $\lambda_{\text{rec}}(X)$ provisional pending uniqueness.
...
## 12. Closure Status Freeze (Phase 1 Canon)

This section is the formal closure specification freeze used by the GU Unique Cosmology Closure Plan.
It defines admissible function spaces and the only allowed status labels.

### 12.1 Allowed Status Labels

- **DERIVED**: unique from GU laws and fixed constants, with no hidden free dial in production.
- **CONSTRAINED**: structure fixed, but one or more free dials remain after consistency constraints.
- **CHOSEN**: model-selection choice (used for scans), not uniquely implied by GU.
- **PROVISIONAL**: working form used only with an explicit gap proof and falsification tests.

### 12.2 Admissible Function Spaces

For closure targets `beta(X)`, `lambda_rec(X)`, `g_OmegaX(X)`, `V_X(X)`, we restrict to:

1. **Regularity**: `C^1` over production domain and no singularities at finite positive `X`.
2. **Stability**: `V_eff(X, M) > 0` during inflation and bounded-below energy density.
3. **Reheating floor**: interaction-induced reheating must satisfy `T_reh > 0.2 GeV`.
4. **Late-time decoupling**: interaction/memory corrections must not spoil standard late-time cosmology.
5. **Observational viability**: at least one closure mode must satisfy current `A_s, n_s, r, z_rec, eta_B` bounds.

### 12.3 Function-by-Function Initial Gate State

At freeze time, no target function is promoted to DERIVED unless identifiability tests pass:

| Function | Initial gate state | Reason |
|----------|--------------------|--------|
| `beta(X)` | PROVISIONAL | canonical `beta=X` is physically motivated but not yet uniqueness-proven |
| `lambda_rec(X)` | PROVISIONAL | only ratio-level closure `lambda_rec/beta = e^phi/pi^2` is currently fixed |
| `g_OmegaX(X)` | CONSTRAINED | reduced parameterization exists, but free dial remains |
| `V_X(X)` | CHOSEN | viable band exists; unique canonical form not yet derived |

### 12.4 Promotion Rule

A function can be promoted to DERIVED only if:

1. local identifiability (full-rank Jacobian or equivalent) is satisfied in the observable map,
2. no hidden free knob survives in production mode,
3. gate tests pass and are reproducibly logged by code output.

### 12.5 Memory Reconciliation Policy (Cosmology <-> Hamiltonian)

- Canonical ratio law is global: `lambda_rec(X)/beta(X) = e^phi/pi^2`.
- Absolute `lambda_rec(X)` remains gate-controlled and cannot be labeled DERIVED until identifiability + falsification pass.
- `H[Omega] = Omega^dagger Omega` is the cosmology authority form. Legacy Hamiltonian `rho^4` notation is treated as regime-specific composite response.
- Exact nonlocal-kernel and local augmented-state (`M_N`) formulations are both admissible, but each result must explicitly declare reduction assumptions and validity domain.

---

## 13. Summary: What Is Derived vs What Is Chosen vs What Is Open

| Quantity | Status | Source |
|----------|--------|--------|
| $\beta(X) = X$ | PROVISIONAL (canonical ansatz) | Promoted only after uniqueness gate |
| $\lambda_{\text{rec}}(X) = X e^\varphi/\pi^2$ | PROVISIONAL (ratio-closed) | `\lambda_{\text{rec}}/\beta = e^\varphi/\pi^2` fixed; absolute closure gated |
...
| $g_0$ (interaction strength) | CONSTRAINED | Reheating + stability (~1 effective dial) |
...
## 15. Erratum: Demonstration Document Arithmetic Errors
