...
cosmic_epochs = [
    ...
    (143, "Today", float(X_at_epoch(143)), "Dark energy dominates")
]
...
# Relate to X evolution (n_today derived from kappa=1.746 + T_CMB, see 00_n_today_derivation.py)
N_today = 143
X_today = float(X_at_epoch(N_today))
scaling = X_today / X_rec

print(f"\nFrom X evolution:")
print(f"X_today/X_rec = φ^({N_rec}-{N_today}) = φ^({N_rec - N_today}) = {scaling:.3e}")
...
print(f"Answer: N_today ~ 143 (derived) is special epoch where X ~ meV")
