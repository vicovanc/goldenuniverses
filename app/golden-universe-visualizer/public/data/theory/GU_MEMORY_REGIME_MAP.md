...
Current closure-gate status:
- `beta(X)`: provisional
- `lambda_rec(X)`: provisional at absolute level (ratio-level closed)
- `g_{OmegaX}(X)`: constrained
- `V_X(X)`: chosen non-unique

Full-ODE identifiability note: current machine diagnostics remain rank-deficient (`2/5`), so no promotion to DERIVED is allowed yet.

Full-ODE identifiability note: rank remains `2/5`; gate policy blocks DERIVED promotion.
...
Memory in GU is a universal structural channel (`beta = X`, `H[Omega]` closure) whose numerical impact is strongly epoch-dependent: subleading in inflation and recombination fits, potentially important in backreaction, history dependence, and cross-sector consistency.
